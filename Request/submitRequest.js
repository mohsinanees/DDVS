...
function submitRequest(approverPrivateKey, requesterPrivateKey, type, sourceDid, sourceVerKey, destDid, destVerKey, 
  nonce, signature) {
  let record = {
    type: type,
    sourceDid: sourceDid,
  ...
  const client = new RequestTransaction(approverPrivateKey, requesterPrivateKey)
...
function submitAuthorizationRequest(authorizerPrivateKeyHex, requesterPrivateKeyHex, type, sourceDid, sourceVerKey, destDid, destVerKey, schemaID,
  ...
  const client = new RequestTransaction(authorizerPrivateKeyHex, requesterPrivateKeyHex);
  const records = [record];
  const transactions = client.CreateTransactions(records);
  const batch = client.CreateBatch(transactions);
  try{
