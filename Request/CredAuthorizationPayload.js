/*                                                                                         *  
    --------------------------------------------------------------------------------------- 
   | This is the Transaction Structure Definition for The Credential Authorization Request |
   | that will be generated by the "Issuer" to "Authorizer" in order to get the Credential | 
   | Authorized using Cryptographic Digital Signatures.                                    | 
    ---------------------------------------------------------------------------------------  
*                                                                                         */
"use strict";
...
class CredAuthorizationPayload {
    constructor(
        ...
        this.type = type;
        this.requestID = createHash("sha256")
            .update(JSON.stringify([type, sourceDid, destDid]))
            .digest("hex");
    ...
    static async fromBytes(payload) {
        ...
        if (res) {
            ...
            return new CredAuthorizationPayload(
                res.type,
                res.sourceDid,
                res.sourceVerKey,
                res.destDid,
