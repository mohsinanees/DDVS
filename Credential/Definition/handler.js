...
class CredentialHandler extends TransactionHandler {
    ...
    async apply(transactionProcessRequest, context) {
        let payload = await CredentialPayload.fromBytes(
        ...
        let result = await context.getState([
            authorizerAddress,
            issuerAddress,
            schemaAddress,
            requesterAddress,
        ...
        let authorizerStatus = authorizerVerify(
            result,
            authorizerAddress,
            payload.authorizerDid,
            payload.authorizerVerKey,
            payload.authorizerSignature,
            payload.nonce
        );
        if (authorizerStatus) {
            let issuerStatus = issuerVerify(
                ...
                payload.issuerSignature,
                payload.nonce
            );
            if (issuerStatus) {
                let schemaStatus = schemaVerify(
                    result,
                    schemaAddress,
                    payload.authorizerDid,
                    payload.authorizerVerKey,
                ...
                if (schemaStatus) {
                    let requesterStatus = requesterExist(
                        result,
                        requesterAddress
                    );
